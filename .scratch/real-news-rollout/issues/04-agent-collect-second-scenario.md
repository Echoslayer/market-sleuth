...
**What to build:** 優先叫 agy（yolo 模式）搜尋某 ticker 某歷史時段的真實新聞、驗證 URL 存在，產出符合 raw-news.json 形狀（含 issue 03 新增的 url/source=agent）的檔案，走既有 `import-raw-news` 匯入，成為第二關。零 pipeline 程式碼。

**Blocked by:** 03（需要 url/source 欄位）；建議在 01 有結論後做（決定要不要人工複核 agent 評分）
